...
internal data class VrStrobePanelActions(
    ...
    val setDepthDeformationEnabled: (Boolean, String) -> VrStrobeSafetySnapshot,
    val setDepthMaxDisplacementMeters: (Float, String) -> VrStrobeSafetySnapshot,
  ...
  when (screen) {
    ...
    VrStrobePanelScreen.ACTIVE ->
        ActiveStimulus(
            ...
            setDepthDeformationEnabled = { enabled ->
              safetySnapshot =
                  actions.setDepthDeformationEnabled(enabled, "active-panel")
            },
            setDepthMaxDisplacementMeters = { meters ->
              safetySnapshot =
                  actions.setDepthMaxDisplacementMeters(meters, "active-panel")
            },
            store = {
...
private fun ActiveStimulus(
    ...
    setDepthDeformationEnabled: (Boolean) -> Unit,
    setDepthMaxDisplacementMeters: (Float) -> Unit,
    store: () -> Unit,
  ...
  LazyColumn(
    ...
    item {
      SectionTitle("Experimental depth ripple")
      Text(
          when {
            !snapshot.depthDeformationAvailable ->
                "Effective state: OFF — temporal presets never deform. Saved interference setting: ${if (snapshot.depthDeformationEnabled) "ON" else "OFF"}."
            snapshot.depthDeformationEffective ->
                "Current state: ON AND ACTIVE"
            snapshot.depthDeformationEnabled ->
                "Current state: ON — waiting for the black lead-in to finish"
            else ->
                "Current state: OFF"
          },
      ...
      OutlinedButton(
          enabled = snapshot.depthDeformationAvailable,
          onClick = {
            setDepthDeformationEnabled(!snapshot.depthDeformationEnabled)
          },
          modifier = Modifier.fillMaxWidth(),
      ) {
        Text(
            if (snapshot.depthDeformationEnabled) {
              "TURN DEPTH DEFORMATION OFF"
            } else {
              "TURN DEPTH DEFORMATION ON"
            }
        )
      }
      Text(
          if (snapshot.depthDeformationAvailable) {
            "Interference only. Dark values move away and bright values move toward you along the carrier normal."
          } else {
            "Unavailable for temporal presets; temporal output always stays geometrically flat."
          }
      )
      Text(
          "Maximum displacement: ±${"%.1f".format(Locale.US, snapshot.depthMaxDisplacementMeters * 100f)} cm"
      )
      Slider(
          value =
              VrStrobeDepthDeformationPolicy.sliderPosition(
                  snapshot.depthMaxDisplacementMeters
              ),
          onValueChange = { position ->
            setDepthMaxDisplacementMeters(
                VrStrobeDepthDeformationPolicy.maxMetersFromSlider(position)
            )
          },
          enabled = snapshot.depthDeformationAvailable,
          valueRange = 0f..1f,
      ...
      Text(
          "Range: 0–${"%.0f".format(Locale.US, VrStrobeDepthDeformationPolicy.MAX_MAX_METERS * 100f)} cm (half the projection radius). The nonlinear slider preserves fine control near zero."
      )
      Text(
          "Values above 20 cm are an experimental stress range and may produce strong self-occlusion, faceting, or discomfort. Band-limited 32 × 96 vertex mesh; the fine fragment pattern and single-draw path are unchanged."
      )
    }
    item {
