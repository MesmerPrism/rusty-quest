...
internal data class VrStrobeCarrierNormal(val x: Float, val y: Float, val z: Float)
...
internal object VrStrobeCarrierGeometry {
  ...
  const val RADIAL_RINGS = 32
  const val ANGULAR_SEGMENTS = 96
  ...
  /** CPU oracle for the analytic local normal used by vertex displacement. */
  fun viewerFacingNormal(
      point: VrStrobeCarrierPoint,
      shape: VrStrobeCarrierShapeState,
  ): VrStrobeCarrierNormal {
    val sanitized = shape.sanitized()
    val polarAngle =
        if (sanitized.curvedMode) {
          VrStrobeConcavityPolicy.polarAngleRadians(sanitized.concavity)
    ...
    if (polarAngle <= 0.0001f) return VrStrobeCarrierNormal(0f, 0f, 1f)
    val sphereRadius = RADIUS_METERS / sin(polarAngle)
    val x = -point.x
    val y = -point.y
    val z = sphereRadius - point.z
    val length = sqrt(x * x + y * y + z * z).coerceAtLeast(0.0001f)
    return VrStrobeCarrierNormal(x / length, y / length, z / length)
  }
}
