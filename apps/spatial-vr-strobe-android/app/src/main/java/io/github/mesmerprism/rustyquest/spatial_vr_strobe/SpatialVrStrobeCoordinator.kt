...
internal class SpatialVrStrobeCoordinator(
  ...
  private var depthDeformation = VrStrobeDepthDeformationState()
  ...
  fun setDepthDeformationEnabled(
      enabled: Boolean,
      source: String,
  ): VrStrobeSafetySnapshot {
    val nowMs = bindings.monotonicNowMs()
    val current = safety.snapshot(nowMs)
    if (!stimulusSelected() || current.outputKind != VrStrobeOutputKind.INTERFERENCE) {
      return report(
          "depth-deformation-rejected-$source",
          current.copy(rejectionReason = "depth-deformation-requires-interference-stimulus"),
      )
    }
    depthDeformation = depthDeformation.copy(enabled = enabled).sanitized()
    applyDepthDeformation()
    bindings.marker(
        "channel=spatial-vr-strobe status=depth-deformation-toggle " +
            "source=${source.replace(' ', '_')} requestedEnabled=$enabled effectiveEnabled=$enabled " +
            "outputKind=interference maxDisplacementMeters=${depthDeformation.maxDisplacementMeters} " +
            "signedRangeMeters=${-depthDeformation.maxDisplacementMeters}..${depthDeformation.maxDisplacementMeters} " +
            "mapping=palette-relative-slot-order normal=viewer-facing-surface " +
            "mesh=${VrStrobeCarrierGeometry.RADIAL_RINGS}x${VrStrobeCarrierGeometry.ANGULAR_SEGMENTS} " +
            "materialSidedness=front-only fragmentHotPathChanged=false visibleDrawCount=1"
    )
    return report("depth-deformation-toggle-$source", current.copy(rejectionReason = "none"))
  }

  fun setDepthMaxDisplacementMeters(
      maxMeters: Float,
      source: String,
  ): VrStrobeSafetySnapshot {
    val nowMs = bindings.monotonicNowMs()
    ...
    if (!stimulusSelected() || current.outputKind != VrStrobeOutputKind.INTERFERENCE) {
      return report(
          "depth-deformation-amplitude-rejected-$source",
          current.copy(rejectionReason = "depth-deformation-requires-interference-stimulus"),
      )
    }
    val previous = depthDeformation.maxDisplacementMeters
    depthDeformation = depthDeformation.copy(maxDisplacementMeters = maxMeters).sanitized()
    if (depthDeformation.maxDisplacementMeters != previous) {
      applyDepthDeformation()
      bindings.marker(
          "channel=spatial-vr-strobe status=depth-deformation-amplitude " +
              "source=${source.replace(' ', '_')} requestedMeters=$maxMeters " +
              "effectiveMeters=${depthDeformation.maxDisplacementMeters} " +
              "effectiveDepthFrequencyLimit=${VrStrobeDepthDeformationPolicy.spatialFrequencyLimit(depthDeformation.maxDisplacementMeters)} " +
              "enabled=${depthDeformation.enabled} outputKind=interference " +
              "vertexUniformWrites=1 fragmentHotPathChanged=false visibleDrawCount=1"
      )
    }
    return report("depth-deformation-amplitude-$source", current.copy(rejectionReason = "none"))
  }
  ...
  private fun ensureCarrier() {
    ...
    bindings.marker(
        "channel=spatial-vr-strobe status=carrier-created referenceSpace=view-locked " +
            ...
            "concavity=${carrierShape.concavity} vertexDepthDeformation=interference-only-default-on " +
            "depthDeformationEnabled=${depthDeformation.enabled} " +
            "depthMaxDisplacementMeters=${depthDeformation.maxDisplacementMeters} " +
            "depthSpatialFrequencyLimit=${VrStrobeDepthDeformationPolicy.MAX_SPATIAL_FREQUENCY} " +
            "depthSpatialFrequencyAtMaxDisplacement=${VrStrobeDepthDeformationPolicy.MIN_SPATIAL_FREQUENCY_AT_MAX_DISPLACEMENT} " +
            "depthMapping=palette-relative-slot-order materialSidedness=front-only " +
            "gpuApi=spatial-sdk-vulkan " +
  ...
  private fun createCarrierRenderSlot(
    ...
    val ownedMaterial =
        SceneMaterial.custom(VrStrobeFeatureRoute.SHADER_NAME, materialAttributes()).apply {
          setSidedness(MaterialSidedness.FRONT_SIDED)
    ...
    applyDepthDeformation(ownedMaterial)
  ...
  private fun applyDepthDeformation(target: SceneMaterial? = material) {
    val sanitized = depthDeformation.sanitized()
    depthDeformation = sanitized
    target?.let { targetMaterial ->
      setMaterialAttribute(
          targetMaterial,
          "depthDeformation",
          Vector4(
              sanitized.enabled.floatValue(),
              sanitized.maxDisplacementMeters,
              VrStrobeDepthDeformationPolicy.MAX_SPATIAL_FREQUENCY,
              VrStrobeDepthDeformationPolicy.MIN_SPATIAL_FREQUENCY_AT_MAX_DISPLACEMENT,
          ),
      )
    }
  }
  ...
  private fun decorate(snapshot: VrStrobeSafetySnapshot): VrStrobeSafetySnapshot {
    ...
    return snapshot.copy(
          ...
          depthDeformationAvailable =
              snapshot.outputKind == VrStrobeOutputKind.INTERFERENCE && stimulusSelected(),
          depthDeformationEnabled = depthDeformation.enabled,
          depthDeformationEffective =
              depthDeformation.enabled &&
                  snapshot.outputKind == VrStrobeOutputKind.INTERFERENCE &&
                  snapshot.visualOutputActive,
          depthMaxDisplacementMeters = depthDeformation.maxDisplacementMeters,
