...
final class ConnectionHubSampleProvider {
    interface Listener {
        void onStateChanged(String phase, boolean toggled);
    }
    ...
    ConnectionHubSampleProvider(Context context, Listener listener) {
    ...
    void start() {
        if (started) {
            render(registered ? "surface registered" : "broker connecting");
            return;
        }
        started = true;
    ...
    private static void requireApplied(JSONObject receipt) throws Exception {
        if (!receipt.getBoolean("applied")) {
            throw new SecurityException("admission rejected");
        }
    }

    private void render(String phase) {
        listener.onStateChanged(phase, toggled);
    }
