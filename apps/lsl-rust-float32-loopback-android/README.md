# Rusty LSL Float32 Loopback Android Harness

This public test-only app builds the accepted Rusty LSL crate for
`aarch64-linux-android` and runs one finite, one-channel, one-record Float32
outlet/inlet exchange over IPv4 loopback inside Rust. Java owns Android
...
chunk, broad compatibility, production activation, or Manifold evidence.

Use `tools/Build-LslRustFloat32LoopbackAndroid.ps1` with an exact clean Rusty
LSL root, then `tools/Invoke-LslRustFloat32LoopbackQuest.ps1` with one explicit
