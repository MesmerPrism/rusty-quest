...
void writeRaymarchedDepth(vec3 hitPoint) {
  vec4 worldHit = g_PrimitiveUniform.worldFromObject * vec4(hitPoint, 1.0);
  vec4 clipHit = getClipFromWorld() * worldHit;
