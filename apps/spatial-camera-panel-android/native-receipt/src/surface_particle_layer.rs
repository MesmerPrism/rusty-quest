...
pub extern "system" fn Java_io_github_mesmerprism_rustyquest_spatial_1camera_1panel_SpatialCameraPanelActivity_nativeUpdateSurfaceParticlePanelPose(
    ...
    if update_count < 4 {
        android_log_info(
            "RQSpatialCameraPanelNative",
            &format!(
                "RUSTY_QUEST_SPATIAL_CAMERA_PANEL_NATIVE channel=native-surface-particle-layer status=panel-pose-updated renderPolicy=native-vulkan-wsi-surface-panel panelPoseNativeUpdate=true worldToPanelProjection=spatial-sdk-panel-plane-basis particleLayerTargetDistanceParameterSource=runtime-hotload-android-property panelCenterM={:.4};{:.4};{:.4} panelRight={:.4};{:.4};{:.4} panelUp={:.4};{:.4};{:.4} panelWidthMeters={:.3} panelHeightMeters={:.3} panelTargetDistanceMeters={:.3}",
                projection.center[0],
                projection.center[1],
                projection.center[2],
                projection.right[0],
