...
impl SpatialVideoProjectionFrameStats {
...
impl SpatialVideoProjectionRenderer {
    pub(crate) unsafe fn new(
        ...
        log_marker(format!(
            "status=renderer-created importSupported={} sameSurfaceComposition=true composedBeforeCamera=true",
            import_supported
        ));
        Self {
            ahb,
    ...
    pub(crate) unsafe fn prepare_frame(
        ...
        settings: &SpatialVideoProjectionSettings,
    ) -> Result<Option<PreparedSpatialVideoProjection>, String> {
        if !settings.active() {
            return Ok(None);
        }
            ...
            self.imports.iter().position(|import| import.key == key)
        {
            ...
            let resources = self.resources.as_ref().ok_or_else(|| {
                "spatial video projection resources were not initialized".to_string()
            })?;
            let mut import = import_spatial_video_projection_hardware_buffer(
                device,
    ...
    pub(crate) unsafe fn record_video_eye(
        ...
        eye_index: usize,
        settings: &SpatialVideoProjectionSettings,
        prepared: &PreparedSpatialVideoProjection,
    ) {
        let target_rect = settings.target_rect_for_eye(eye_index);
        ...
        let source_uv_rect = settings.source_rect_for_eye(eye_index);
        let push = SpatialVideoProjectionPush {
            target_rect,
            source_uv_rect,
            params0: [0.0, settings.opacity, 0.0, 0.0],
        };
        let push_bytes = std::slice::from_raw_parts(
            (&push as *const SpatialVideoProjectionPush).cast::<u8>(),
            std::mem::size_of::<SpatialVideoProjectionPush>(),
        );
...
struct SpatialVideoProjectionCacheEvictionStats {
    attempts: usize,
    applied: usize,
...
impl SpatialVideoProjectionCacheEvictionStats {
...
impl SpatialVideoProjectionResources {
...
impl SpatialVideoProjectionImport {
    unsafe fn destroy(self, device: &ash::Device) {
...
#[repr(C)]
#[derive(Clone, Copy)]
struct SpatialVideoProjectionPush {
    target_rect: [f32; 4],
    source_uv_rect: [f32; 4],
    params0: [f32; 4],
}

unsafe fn create_spatial_video_projection_resources(
    device: &ash::Device,
    render_pass: vk::RenderPass,
    format_key: AhbVulkanFormatKey,
    format_props: &vk::AndroidHardwareBufferFormatPropertiesANDROID<'_>,
) -> Result<SpatialVideoProjectionResources, String> {
    let sampler_ycbcr_conversion = create_ahb_sampler_ycbcr_conversion(
        device,
        format_key,
        format_props,
    ...
    let sampler = match device.create_sampler(&sampler_info, None) {
        ...
        Err(error) => {
            ...
            return Err(format!("create-spatial-video-projection-sampler-{error:?}"));
        }
    };
    ...
    let descriptor_set_layout = match device.create_descriptor_set_layout(
        ...
        Err(error) => {
            ...
            return Err(format!(
                "create-spatial-video-projection-descriptor-set-layout-{error:?}"
            ));
        }
    };
    ...
    let descriptor_pool = match device.create_descriptor_pool(
        ...
        Err(error) => {
            ...
            return Err(format!(
                "create-spatial-video-projection-descriptor-pool-{error:?}"
            ));
        }
    };
    let push_ranges = [vk::PushConstantRange::default()
        .stage_flags(vk::ShaderStageFlags::FRAGMENT)
        .offset(0)
        .size(std::mem::size_of::<SpatialVideoProjectionPush>() as u32)];
    ...
    let pipeline_layout = match device.create_pipeline_layout(
        ...
        Err(error) => {
            ...
            return Err(format!(
                "create-spatial-video-projection-pipeline-layout-{error:?}"
            ));
        }
    };
    let pipeline =
        match create_spatial_video_projection_pipeline(device, render_pass, pipeline_layout) {
...
unsafe fn allocate_spatial_video_projection_descriptor_set(
    device: &ash::Device,
    resources: &SpatialVideoProjectionResources,
    ...
    let descriptor_set = device
        ...
        .map_err(|error| format!("allocate-spatial-video-projection-descriptor-set-{error:?}"))?
        .pop()
        .ok_or_else(|| "spatial-video-projection-descriptor-allocation-empty".to_string())?;
...
unsafe fn import_spatial_video_projection_hardware_buffer(
    device: &ash::Device,
    memory_properties: &vk::PhysicalDeviceMemoryProperties,
    resources: &SpatialVideoProjectionResources,
    frame: &SpatialVideoProjectionFrame,
...
) -> Result<SpatialVideoProjectionImport, String> {
    let width = frame
        .descriptor
        .width
    ...
    let sampled_image = import_ahb_sampled_image(
        ...
        AhbVulkanSampledImageCreateInfo {
            ...
            debug_label: "spatial-video-projection",
        },
    )?;
    let descriptor_set = allocate_spatial_video_projection_descriptor_set(
        device,
        resources,
...
unsafe fn create_spatial_video_projection_pipeline(
    device: &ash::Device,
    render_pass: vk::RenderPass,
    pipeline_layout: vk::PipelineLayout,
    ...
    let fragment_module = match create_shader_module(
        device,
        include_bytes!(concat!(
            env!("OUT_DIR"),
    ...
    pipeline_result
        ...
        .map_err(|(_, error)| format!("create-spatial-video-projection-pipeline-{error:?}"))
}
...
unsafe fn create_shader_module(
    ...
    if bytes.len() % std::mem::size_of::<u32>() != 0 {
        return Err("spatial-video-projection-shader-bytes-not-u32-aligned".to_string());
    }
    ...
    device
        ...
        .map_err(|error| format!("create-spatial-video-projection-shader-module-{error:?}"))
}
