...
const SPATIAL_PUBLIC_GUIDE_PREBLUR_KERNEL_PROPERTY: &str =
    "debug.rustyquest.spatial.camera_hwb_projection_probe.guide.preblur.kernel";
const SPATIAL_PUBLIC_GUIDE_PREBLUR_INPUT_PROPERTY: &str =
    "debug.rustyquest.spatial.camera_hwb_projection_probe.guide.preblur.input";
const SPATIAL_PUBLIC_GUIDE_POSTBLUR_KERNEL_PROPERTY: &str =
...
const SPATIAL_PUBLIC_DEPTH_ALIGNMENT_ROLL_DEGREES_MIN: f32 = -15.0;
const SPATIAL_PUBLIC_DEPTH_ALIGNMENT_ROLL_DEGREES_MAX: f32 = 15.0;
const SPATIAL_PUBLIC_DEPTH_AFFINE_SAMPLE_DELTA: f32 = 0.01;
...
static SPATIAL_PUBLIC_DEPTH_ALIGNMENT_SAMPLE_SCALE_Y_BITS: AtomicU32 =
    AtomicU32::new(SPATIAL_PUBLIC_DEPTH_ALIGNMENT_SAMPLE_SCALE_DEFAULT.to_bits());
...
impl SpatialPublicDepthLayerPolicy {
    ...
    fn source_view_index_for_eye(self, eye_index: usize) -> usize {
        match self {
            Self::MonoLayer0 => 0,
            Self::MonoLayer1 => 1,
            Self::EyeIndex | Self::Compare => eye_index.min(SPATIAL_PUBLIC_PACKED_EYE_COUNT - 1),
        }
    }
}

#[derive(Clone, Copy, Debug, Default)]
struct SpatialPublicDepthFovSnapshot {
    angle_left: f32,
    angle_right: f32,
    angle_up: f32,
    angle_down: f32,
...
struct SpatialPublicDepthPoseSnapshot {
    orientation: [f32; 4],
    position: [f32; 3],
}

impl Default for SpatialPublicDepthPoseSnapshot {
    fn default() -> Self {
        Self {
...
pub(crate) struct SpatialPublicDepthAlignment {
    ...
    pub(crate) sample_scale_y: f32,
    pub(crate) roll_degrees: f32,
    pub(crate) metadata_auto_align: bool,
}

impl SpatialPublicDepthAlignment {
    fn depth_uv_affine_for_eye(
        self,
        eye_index: usize,
...
pub(crate) fn spatial_public_guide_target_extent() -> vk::Extent2D {
    vk::Extent2D {
        width: SPATIAL_PUBLIC_GUIDE_PER_EYE_WIDTH * SPATIAL_PUBLIC_PACKED_EYE_COUNT as u32,
        height: SPATIAL_PUBLIC_GUIDE_PER_EYE_HEIGHT,
    }
}
...
pub(crate) struct SpatialPublicGuideTargets {
    ...
    opaque_projection_displacement_pipeline: Option<vk::Pipeline>,
...
impl SpatialPublicGuideTargets {
    pub(crate) unsafe fn destroy(self, device: &ash::Device) {
        ...
        if let Some(pipeline) = self.opaque_projection_displacement_pipeline {
            device.destroy_pipeline(pipeline, None);
        }
        if let Some(pipeline) = self.projection_zone_video_pipeline {
    ...
    pub(crate) fn marker_fields(&self) -> String {
        ...
        format!(
            ...
            self.extent.width,
            self.extent.height,
            SPATIAL_PUBLIC_GUIDE_PER_EYE_WIDTH,
            SPATIAL_PUBLIC_GUIDE_PER_EYE_HEIGHT,
            self.format,
            total_bytes,
            memory_policy,
    ...
    pub(crate) fn compact_projection_evidence_marker_fields(
        ...
        format!(
            ...
            depth_alignment.sample_scale_y,
            depth_alignment.roll_degrees,
            bool_marker(depth_alignment.metadata_auto_align),
        )
        + " "
        + &current_projection_surface_displacement_settings()
            .marker_fields(self.opaque_projection_displacement_pipeline.is_some())
    ...
    pub(crate) fn compact_depth_evidence_marker_fields(&self) -> Option<String> {
        let current = self.depth_resources.current_binding();
    ...
    pub(crate) fn compact_depth_alignment_evidence_marker_fields(&self) -> Option<String> {
        let current = self.depth_resources.current_binding();
        if !current.real_depth_bound {
            return None;
        }
        ...
        Some(format!(
            ...
            right_affine.row1[1],
            right_affine.row1[2],
            alignment.left_offset_uv[0],
            alignment.left_offset_uv[1],
            alignment.right_offset_uv[0],
            alignment.right_offset_uv[1],
    ...
    pub(crate) fn compact_depth_source_evidence_marker_fields(&self) -> Option<String> {
        self.depth_resources
            .current_binding()
            .real_depth_bound
            .then(spatial_environment_depth_compact_marker_fields)
    }
    ...
    pub(crate) unsafe fn record_spatial_public_guide_passes(
            ...
            .take(guide_pass_count)
        {
            match step.kind {
                ...
                SpatialPublicGuidePassKind::PublicBlur { direction, stage } => {
                    let source_target = step
                        .input_target
                        .ok_or_else(|| "public-blur-source-target-missing".to_string())?;
                    let pass_policy = current_spatial_guide_processing_policy().for_stage(stage);
    ...
    pub(crate) unsafe fn prepare_projection_zone_compositor(
        &mut self,
        device: &ash::Device,
    ...
    pub(crate) unsafe fn record_spatial_public_projection_in_open_render_pass(
        ...
        for eye_index in 0..SPATIAL_PUBLIC_PACKED_EYE_COUNT {
            ...
            let pipeline = if tessellated_effective {
                ...
                self.opaque_projection_pipeline
    ...
    pub(crate) unsafe fn record_projection_zone_compositor_in_open_render_pass(
        &self,
        device: &ash::Device,
        command_buffer: vk::CommandBuffer,
        extent: vk::Extent2D,
        camera_descriptor_set: vk::DescriptorSet,
        video_descriptor_set: vk::DescriptorSet,
        elapsed_seconds: f32,
        footprint_scale: f32,
        zone_frame: &CameraHwbProjectionZoneFrame,
    ) -> Result<bool, String> {
        ...
        for eye_index in 0..SPATIAL_PUBLIC_PACKED_EYE_COUNT {
            set_packed_projection_target_view(
                ...
                command_buffer,
                extent,
                zone_frame.draw_rects[eye_index],
            );
    ...
    pub(crate) unsafe fn record_public_blur_pass(
        ...
        let push = PublicGuideBlurPush {
            step_and_scale: direction.step_and_extent(self.extent),
            source_rect: [0.0, 0.0, 1.0, 1.0],
            processing: current_spatial_guide_processing_policy()
                .for_stage(match direction {
                    PublicGuideBlurDirection::Horizontal => SpatialGuideBlurStage::PreHorizontal,
                    PublicGuideBlurDirection::Vertical => SpatialGuideBlurStage::PreVertical,
                })
                .push_codes(),
        };
    ...
    unsafe fn record_public_blur_pass_for_stereo(
        ...
        destination_target_index: usize,
        direction: PublicGuideBlurDirection,
        pass_policy: SpatialGuideBlurPassPolicy,
    ) -> Result<(), String> {
        if source_target_index == destination_target_index {
            return Err("public-guide-blur-source-destination-alias".to_string());
        }
        ...
        for eye_index in 0..SPATIAL_PUBLIC_PACKED_EYE_COUNT {
            ...
            let push = PublicGuideBlurPush {
                ...
                processing: pass_policy.push_codes(),
            };
            push_fragment_constants(device, command_buffer, self.blur_pipeline_layout, &push);
            device.cmd_draw(command_buffer, 3, 1, 0, 0);
        }
    ...
    unsafe fn record_opaque_guide_pass_for_stereo(
        ...
        for eye_index in 0..SPATIAL_PUBLIC_PACKED_EYE_COUNT {
            ...
            let reprojection = if eye_index == 0 {
                camera_reprojection.left
            } else {
                camera_reprojection.right
            };
            let mut reprojection_params = reprojection.params;
            reprojection_params[3] = source_overscan_uv.clamp(0.0, 0.2);
            let camera_sampling = current_spatial_guide_processing_policy().camera_sampling;
            let push = OpaqueGuidePush {
                params0: [
                    eye_index as f32,
                    camera_sampling.opaque_guide_push_flags(),
...
impl PublicGuideBlurDirection {
    fn step_and_extent(self, extent: vk::Extent2D) -> [f32; 4] {
        let width = extent.width.max(1) as f32;
        let height = extent.height.max(1) as f32;
        match self {
            Self::Horizontal => [1.0 / width, 0.0, 1.0 / width, 1.0 / height],
            Self::Vertical => [0.0, 1.0 / height, 1.0 / width, 1.0 / height],
        }
    }
}
...
const _: () = assert!(mem::size_of::<PublicGuideBlurPush>() == 48);
...
const _: () = assert!(mem::size_of::<OpaqueGuidePush>() <= 128);
...
impl OpaqueProjectionPush {
    fn for_packed_eye(
        ...
        Self {
            target_rect: packed_projection_target_rect(eye_index, footprint_scale),
            params0: [
                eye_index as f32,
                0.0,
                elapsed_seconds.max(0.0),
            ...
            border_blend: [
                ...
                SPATIAL_PUBLIC_VIDEO_BORDER_BLEND_CURVE,
                0.0,
            ],
            depth: [
            ...
            depth_aux: [
                ...
                depth_uv_affine.row1[1],
                depth_uv_affine.row1[2],
            ],
            depth_uv_transform: [
                depth_uv_affine.row0[0],
                depth_uv_affine.row0[1],
                depth_uv_affine.row0[2],
                depth_uv_affine.row1[0],
            ],
        }
    }
}
...
pub(crate) fn update_spatial_public_opaque_projection_layer_override(layer_override: f32) -> f32 {
    let applied = clamp_projection_layer_override(presentation_layer_override(layer_override));
...
pub(crate) fn current_spatial_public_opaque_projection_layer_override() -> f32 {
    clamp_projection_layer_override(presentation_layer_override(f32::from_bits(
        ...
        .map(|eye_index| {
            vk::ClearRect::default()
                .rect(target_rect_to_scissor(
                    extent,
                    packed_projection_target_rect(eye_index, footprint_scale),
                ))
                .base_array_layer(0)
                .layer_count(1)
        })
...
pub(crate) fn update_spatial_public_guide_processing_policy(
    preblur_kernel_code: u32,
    preblur_input_code: u32,
    postblur_kernel_code: u32,
    camera_sampling_code: u32,
) -> SpatialGuideProcessingPolicy {
    update_spatial_guide_processing_policy(SpatialGuideProcessingPolicy::from_codes(
        preblur_kernel_code,
        preblur_input_code,
        postblur_kernel_code,
        camera_sampling_code,
    ))
}

fn initialize_spatial_public_guide_processing_policy_from_properties() {
    #[cfg(target_os = "android")]
    {
        let mut policy = current_spatial_guide_processing_policy();
        if let Some(kernel) = android_system_property(SPATIAL_PUBLIC_GUIDE_PREBLUR_KERNEL_PROPERTY)
            .and_then(|raw| SpatialGuideBlurKernel::parse(&raw))
        {
            policy.preblur_kernel = kernel;
        }
        if let Some(input) = android_system_property(SPATIAL_PUBLIC_GUIDE_PREBLUR_INPUT_PROPERTY)
            .and_then(|raw| SpatialGuideInputTreatment::parse(&raw))
        {
            policy.preblur_input = input;
        }
        if let Some(kernel) = android_system_property(SPATIAL_PUBLIC_GUIDE_POSTBLUR_KERNEL_PROPERTY)
            .and_then(|raw| SpatialGuideBlurKernel::parse(&raw))
        {
            policy.postblur_kernel = kernel;
        }
        if let Some(sampling) = android_system_property(SPATIAL_PUBLIC_CAMERA_SAMPLING_PROPERTY)
            .and_then(|raw| SpatialCameraSampling::parse(&raw))
        {
            policy.camera_sampling = sampling;
        }
        update_spatial_guide_processing_policy(policy);
    }
}
...
pub(crate) fn update_spatial_public_depth_alignment(
    ...
    let alignment = SpatialPublicDepthAlignment {
        ...
        sample_scale_y: clamp_depth_alignment_sample_scale(sample_scale_y),
        roll_degrees: clamp_depth_alignment_roll_degrees(roll_degrees),
        metadata_auto_align,
    };
    ...
    SPATIAL_PUBLIC_DEPTH_ALIGNMENT_SAMPLE_SCALE_Y_BITS
        .store(alignment.sample_scale_y.to_bits(), Ordering::Release);
    SPATIAL_PUBLIC_DEPTH_ALIGNMENT_ROLL_DEGREES_BITS
        .store(alignment.roll_degrees.to_bits(), Ordering::Release);
    SPATIAL_PUBLIC_DEPTH_ALIGNMENT_METADATA_AUTO
        .store(alignment.metadata_auto_align, Ordering::Release);
    alignment
}
...
pub(crate) fn current_spatial_public_depth_alignment() -> SpatialPublicDepthAlignment {
    SpatialPublicDepthAlignment {
        ...
        sample_scale_y: clamp_depth_alignment_sample_scale(f32::from_bits(
            SPATIAL_PUBLIC_DEPTH_ALIGNMENT_SAMPLE_SCALE_Y_BITS.load(Ordering::Acquire),
        )),
        roll_degrees: clamp_depth_alignment_roll_degrees(f32::from_bits(
            SPATIAL_PUBLIC_DEPTH_ALIGNMENT_ROLL_DEGREES_BITS.load(Ordering::Acquire),
        )),
        metadata_auto_align: SPATIAL_PUBLIC_DEPTH_ALIGNMENT_METADATA_AUTO.load(Ordering::Acquire),
    }
}
...
fn opaque_projection_layer_override() -> f32 {
    ...
    #[cfg(target_os = "android")]
    {
        ...
        {
            return presentation_layer_override(value);
        }
    }
    presentation_layer_override(SPATIAL_PUBLIC_OPAQUE_PROJECTION_LAYER_OVERRIDE_DEFAULT)
}
...
fn clamp_depth_alignment_roll_degrees(value: f32) -> f32 {
    if value.is_finite() {
        value.clamp(
            SPATIAL_PUBLIC_DEPTH_ALIGNMENT_ROLL_DEGREES_MIN,
            SPATIAL_PUBLIC_DEPTH_ALIGNMENT_ROLL_DEGREES_MAX,
        )
    } else {
        0.0
    }
...
fn spatial_public_metadata_depth_affine(
    ...
    let render_eye_index = render_eye_index.min(SPATIAL_PUBLIC_PACKED_EYE_COUNT - 1);
    let depth_view_index = policy.source_view_index_for_eye(render_eye_index);
    let render_bit = 1_u32 << render_eye_index;
    let depth_bit = 1_u32 << depth_view_index;
    if !depth_binding.real_depth_bound
        || depth_binding.render_view_valid_mask & render_bit == 0
        || depth_binding.depth_view_valid_mask & depth_bit == 0
    {
        return SpatialPublicDepthUvAffine::identity();
    }
    let render_view = depth_binding.render_views[render_eye_index];
    let depth_view = depth_binding.depth_views[depth_view_index];
    let center = [0.5, 0.5];
    let sample_x = [
        center[0] + SPATIAL_PUBLIC_DEPTH_AFFINE_SAMPLE_DELTA,
        center[1],
    ];
    let sample_y = [
        center[0],
        center[1] + SPATIAL_PUBLIC_DEPTH_AFFINE_SAMPLE_DELTA,
    ];
    let Some(mapped_center) = spatial_public_map_view_uv(render_view, depth_view, center) else {
        return SpatialPublicDepthUvAffine::identity();
    };
    let Some(mapped_x) = spatial_public_map_view_uv(render_view, depth_view, sample_x) else {
        return SpatialPublicDepthUvAffine::identity();
    };
    let Some(mapped_y) = spatial_public_map_view_uv(render_view, depth_view, sample_y) else {
        return SpatialPublicDepthUvAffine::identity();
    };
    let inverse_delta = 1.0 / SPATIAL_PUBLIC_DEPTH_AFFINE_SAMPLE_DELTA;
    let m00 = (mapped_x[0] - mapped_center[0]) * inverse_delta;
    let m10 = (mapped_x[1] - mapped_center[1]) * inverse_delta;
    let m01 = (mapped_y[0] - mapped_center[0]) * inverse_delta;
    let m11 = (mapped_y[1] - mapped_center[1]) * inverse_delta;
    let tx = mapped_center[0] - m00 * center[0] - m01 * center[1];
    let ty = mapped_center[1] - m10 * center[0] - m11 * center[1];
    let values = [m00, m01, tx, m10, m11, ty];
...
fn spatial_public_depth_pose_translation_delta_m(
    render_eye_index: usize,
    policy: SpatialPublicDepthLayerPolicy,
    depth_binding: SpatialPublicDepthBinding,
) -> f32 {
    let render_eye_index = render_eye_index.min(SPATIAL_PUBLIC_PACKED_EYE_COUNT - 1);
    let depth_view_index = policy.source_view_index_for_eye(render_eye_index);
    let render_position = depth_binding.render_views[render_eye_index].pose.position;
    let depth_position = depth_binding.depth_views[depth_view_index].pose.position;
    let delta = [
        depth_position[0] - render_position[0],
        depth_position[1] - render_position[1],
        depth_position[2] - render_position[2],
    ];
    let magnitude = delta.iter().map(|value| value * value).sum::<f32>().sqrt();
...
fn compose_spatial_public_manual_depth_alignment(
    metadata: SpatialPublicDepthUvAffine,
    offset: [f32; 2],
    scale_x: f32,
    scale_y: f32,
    ...
    let (sin_roll, cos_roll) = radians.sin_cos();
    let manual00 = cos_roll * scale_x;
    let manual01 = -sin_roll * scale_y;
    let manual10 = sin_roll * scale_x;
    let manual11 = cos_roll * scale_y;
    let m00 = manual00 * metadata.row0[0] + manual01 * metadata.row1[0];
    let m01 = manual00 * metadata.row0[1] + manual01 * metadata.row1[1];
    let m10 = manual10 * metadata.row0[0] + manual11 * metadata.row1[0];
    let m11 = manual10 * metadata.row0[1] + manual11 * metadata.row1[1];
    let centered_x = metadata.row0[2] - 0.5;
    let centered_y = metadata.row1[2] - 0.5;
    let tx = manual00 * centered_x + manual01 * centered_y + 0.5 + offset[0];
    let ty = manual10 * centered_x + manual11 * centered_y + 0.5 + offset[1];
    SpatialPublicDepthUvAffine {
        row0: [m00, m01, tx],
        row1: [m10, m11, ty],
        metadata_applied: metadata.metadata_applied,
    }
}

fn spatial_public_map_view_uv(
    render_view: SpatialPublicDepthViewSnapshot,
    depth_view: SpatialPublicDepthViewSnapshot,
    render_uv: [f32; 2],
) -> Option<[f32; 2]> {
    let render_ray = spatial_public_fov_ray(render_view.fov, render_uv)?;
    let render_orientation = normalize_quaternion(render_view.pose.orientation)?;
    let depth_orientation = normalize_quaternion(depth_view.pose.orientation)?;
    let reference_ray = rotate_vector_by_quaternion(render_ray, render_orientation);
    let depth_ray = rotate_vector_by_quaternion(
        reference_ray,
        [
            -depth_orientation[0],
            -depth_orientation[1],
    ...
    let forward = -depth_ray[2];
    if !forward.is_finite() || forward <= 0.0001 {
        return None;
    }
    let tan_left = depth_view.fov.angle_left.tan();
    let tan_right = depth_view.fov.angle_right.tan();
    let tan_down = depth_view.fov.angle_down.tan();
    let tan_up = depth_view.fov.angle_up.tan();
    let width = tan_right - tan_left;
    let height = tan_up - tan_down;
    if !width.is_finite() || !height.is_finite() || width.abs() <= 0.0001 || height.abs() <= 0.0001
    {
        return None;
    }
    let depth_tangent_x = depth_ray[0] / forward;
    let depth_tangent_y = depth_ray[1] / forward;
    let mapped = [
        (depth_tangent_x - tan_left) / width,
        (depth_tangent_y - tan_down) / height,
    ];
    mapped
        .iter()
        .all(|value| value.is_finite())
...
fn spatial_public_fov_ray(fov: SpatialPublicDepthFovSnapshot, uv: [f32; 2]) -> Option<[f32; 3]> {
    let tangents = [
        fov.angle_left.tan(),
        fov.angle_right.tan(),
        fov.angle_down.tan(),
    ...
    if !tangents.iter().all(|value| value.is_finite())
    ...
    Some([
        tangents[0] + (tangents[1] - tangents[0]) * uv[0],
        tangents[2] + (tangents[3] - tangents[2]) * uv[1],
        -1.0,
    ])
}
...
fn normalize_quaternion(quaternion: [f32; 4]) -> Option<[f32; 4]> {
    ...
    if !norm_squared.is_finite() || norm_squared <= 0.0001 {
        return None;
    }
    let inverse_norm = norm_squared.sqrt().recip();
    Some(quaternion.map(|value| value * inverse_norm))
}

fn rotate_vector_by_quaternion(vector: [f32; 3], quaternion: [f32; 4]) -> [f32; 3] {
    let q = [quaternion[0], quaternion[1], quaternion[2]];
    let uv = cross3(q, vector);
    let uuv = cross3(q, uv);
    [
...
const SPATIAL_PUBLIC_GUIDE_PASS_SCHEDULE: [SpatialPublicGuidePassStep; 6] = [
    ...
    SpatialPublicGuidePassStep {
        ...
        kind: SpatialPublicGuidePassKind::PublicBlur {
            ...
            stage: SpatialGuideBlurStage::PreHorizontal,
        },
    },
    SpatialPublicGuidePassStep {
        input_target: Some(1),
        ...
        kind: SpatialPublicGuidePassKind::PublicBlur {
            ...
            stage: SpatialGuideBlurStage::PreVertical,
        },
    },
    SpatialPublicGuidePassStep {
        input_target: Some(2),
    ...
    SpatialPublicGuidePassStep {
        ...
        kind: SpatialPublicGuidePassKind::PublicBlur {
            direction: PublicGuideBlurDirection::Horizontal,
            stage: SpatialGuideBlurStage::PostHorizontal,
        },
    },
    SpatialPublicGuidePassStep {
        input_target: Some(1),
        ...
        kind: SpatialPublicGuidePassKind::PublicBlur {
            direction: PublicGuideBlurDirection::Vertical,
            stage: SpatialGuideBlurStage::PostVertical,
        },
    },
];
...
struct SpatialPublicDepthBinding {
    ...
    near_z: f32,
    far_z: f32,
    depth_views: [SpatialPublicDepthViewSnapshot; 2],
    render_views: [SpatialPublicDepthViewSnapshot; 2],
...
impl SpatialPublicDepthResources {
    ...
    fn current_binding(&self) -> SpatialPublicDepthBinding {
        if let Some(snapshot) = current_spatial_environment_depth_frame_snapshot() {
                ...
                .copied()
            {
                return SpatialPublicDepthBinding {
                    ...
                    depth_views: snapshot.depth_views,
                    render_views: snapshot.render_views,
        ...
        SpatialPublicDepthBinding {
            ...
            depth_views: [SpatialPublicDepthViewSnapshot::default(); 2],
            render_views: [SpatialPublicDepthViewSnapshot::default(); 2],
            depth_view_valid_mask: 0,
            render_view_valid_mask: 0,
        }
    }
    ...
    fn marker_fields(&self) -> String {
        ...
        let (
            ...
            (
                ...
                snapshot.acquired_frame_count,
                snapshot.capture_time_ns,
                snapshot.acquire_display_time_ns,
                snapshot.depth_view_valid_mask,
                snapshot.render_view_valid_mask,
            )
        } else {
            ("none".to_string(), "0x0".to_string(), 0, 0, 0, 0, 0)
        };
        let alignment = current_spatial_public_depth_alignment();
        let policy = current_spatial_public_depth_layer_policy();
        ...
        let left_translation_delta_m =
            spatial_public_depth_pose_translation_delta_m(0, policy, current);
        ...
        format!(
            ...
            left_translation_delta_m,
            right_translation_delta_m,
            left_affine.row0[0],
            left_affine.row0[1],
            left_affine.row0[2],
            left_affine.row1[0],
...
fn current_spatial_environment_depth_frame_snapshot(
    ...
    crate::spatial_environment_depth::spatial_environment_depth_frame_snapshot().map(|snapshot| {
        SpatialPublicEnvironmentDepthSnapshot {
            ...
            acquire_display_time_ns: snapshot.acquire_display_time_ns,
...
pub(crate) unsafe fn allocate_spatial_public_guide_targets(
    ...
    initialize_spatial_public_guide_processing_policy_from_properties();
    ...
    let opaque_projection_displacement_pipeline = match create_projection_displacement_pipeline(
        device,
        projection_render_pass,
        opaque_projection_pipeline_layout,
        include_bytes!(concat!(
        ...
        Ok(pipeline) => pipeline,
        Err(error) => {
            if let Some(pipeline) = opaque_projection_pipeline {
                device.destroy_pipeline(pipeline, None);
            }
            for target in targets {
                target.destroy(device);
    ...
    let projection_zone_uniform =
        match create_projection_zone_uniform_resources(device, memory_properties) {
            Ok(resources) => resources,
            Err(error) => {
                if let Some(pipeline) = opaque_projection_displacement_pipeline {
                    device.destroy_pipeline(pipeline, None);
                }
                if let Some(pipeline) = opaque_projection_pipeline {
                    device.destroy_pipeline(pipeline, None);
    ...
    Ok(SpatialPublicGuideTargets {
        ...
        opaque_projection_displacement_pipeline,
        projection_render_pass,
        camera_descriptor_set_layout,
    ...
    SPATIAL_PUBLIC_GUIDE_PASS_SCHEDULE
        ...
        .map(|(index, step)| {
            ...
            let kind = match step.kind {
                ...
                SpatialPublicGuidePassKind::PublicBlur { stage, .. } => match stage {
                    SpatialGuideBlurStage::PreHorizontal => "public-preblur-horizontal".to_string(),
                    SpatialGuideBlurStage::PreVertical => "public-preblur-vertical".to_string(),
                    SpatialGuideBlurStage::PostHorizontal => {
                        "public-postblur-horizontal".to_string()
                    }
                    SpatialGuideBlurStage::PostVertical => "public-postblur-vertical".to_string(),
                },
            };
...
fn packed_projection_target_rect(eye_index: usize, footprint_scale: f32) -> [f32; 4] {
    let push = crate::camera_hwb_projection_target::camera_hwb_projection_push(footprint_scale);
    if eye_index == 0 {
        push.left_rect
    } else {
        push.right_rect
...
unsafe fn create_rgb_channel_transform_uniform_resources(
    ...
    let rgb_buffer_info = [vk::DescriptorBufferInfo::default()
        .buffer(buffer)
        .offset(0)
        .range(size)];
    let displacement_buffer_info = [vk::DescriptorBufferInfo::default()
        .buffer(displacement_buffer)
        .offset(0)
        .range(displacement_size)];
    let writes = [
        vk::WriteDescriptorSet::default()
            .dst_set(descriptor_set)
            .dst_binding(0)
            .descriptor_type(vk::DescriptorType::UNIFORM_BUFFER)
            .buffer_info(&rgb_buffer_info),
        vk::WriteDescriptorSet::default()
            .dst_set(descriptor_set)
            .dst_binding(1)
            .descriptor_type(vk::DescriptorType::UNIFORM_BUFFER)
...
unsafe fn create_host_coherent_uniform_buffer(
    device: &ash::Device,
    memory_properties: &vk::PhysicalDeviceMemoryProperties,
    size: vk::DeviceSize,
    label: &str,
    ...
    let buffer = device
        ...
        .map_err(|error| format!("create-{label}-uniform-buffer-{error:?}"))?;
    let requirements = device.get_buffer_memory_requirements(buffer);
    let Some(memory_type_index) = find_memory_type_index(
        ...
        device.destroy_buffer(buffer, None);
        return Err(format!("{label}-uniform-host-coherent-memory-unavailable"));
    };
    let memory = match device.allocate_memory(
        &vk::MemoryAllocateInfo::default()
            .allocation_size(requirements.size)
        ...
        Err(error) => {
            device.destroy_buffer(buffer, None);
            return Err(format!("allocate-{label}-uniform-memory-{error:?}"));
        }
    };
    if let Err(error) = device.bind_buffer_memory(buffer, memory, 0) {
        device.free_memory(memory, None);
        device.destroy_buffer(buffer, None);
        return Err(format!("bind-{label}-uniform-memory-{error:?}"));
    }
    Ok((buffer, memory))
}
...
unsafe fn create_projection_zone_uniform_resources(
    device: &ash::Device,
    memory_properties: &vk::PhysicalDeviceMemoryProperties,
) -> Result<SpatialProjectionZoneUniformResources, String> {
    ...
    let descriptor_set_layout = device
        ...
        .map_err(|error| format!("create-projection-zone-uniform-layout-{error:?}"))?;
    let size = mem::size_of::<ProjectionZoneUniform>() as vk::DeviceSize;
    let buffer = match device.create_buffer(
        &vk::BufferCreateInfo::default()
            .size(size)
        ...
        Err(error) => {
            device.destroy_descriptor_set_layout(descriptor_set_layout, None);
            return Err(format!("create-projection-zone-uniform-buffer-{error:?}"));
        }
    };
    let requirements = device.get_buffer_memory_requirements(buffer);
    let memory_type_index = match find_memory_type_index(
        ...
        None => {
            device.destroy_buffer(buffer, None);
            device.destroy_descriptor_set_layout(descriptor_set_layout, None);
            return Err("projection-zone-uniform-host-coherent-memory-unavailable".to_string());
        }
    };
    let memory = match device.allocate_memory(
        &vk::MemoryAllocateInfo::default()
        ...
        Err(error) => {
            device.destroy_buffer(buffer, None);
            device.destroy_descriptor_set_layout(descriptor_set_layout, None);
            return Err(format!("allocate-projection-zone-uniform-memory-{error:?}"));
        }
    };
    if let Err(error) = device.bind_buffer_memory(buffer, memory, 0) {
        device.free_memory(memory, None);
        device.destroy_buffer(buffer, None);
        device.destroy_descriptor_set_layout(descriptor_set_layout, None);
        return Err(format!("bind-projection-zone-uniform-memory-{error:?}"));
    }
    let pool_sizes = [vk::DescriptorPoolSize::default()
        .ty(vk::DescriptorType::UNIFORM_BUFFER)
        .descriptor_count(1)];
    let descriptor_pool = match device.create_descriptor_pool(
        ...
        Err(error) => {
            ...
            device.destroy_buffer(buffer, None);
            device.destroy_descriptor_set_layout(descriptor_set_layout, None);
            return Err(format!("create-projection-zone-uniform-pool-{error:?}"));
        }
    };
    let set_layouts = [descriptor_set_layout];
    let descriptor_set = match device.allocate_descriptor_sets(
...
unsafe fn create_projection_zone_video_pipeline(
    device: &ash::Device,
    render_pass: vk::RenderPass,
    camera_descriptor_set_layout: vk::DescriptorSetLayout,
    opaque_guide_descriptor_set_layout: vk::DescriptorSetLayout,
    ...
    let pipeline_layout = device
        ...
        .map_err(|error| format!("create-projection-zone-video-pipeline-layout-{error:?}"))?;
    let fragment_spirv = include_bytes!(concat!(
        env!("OUT_DIR"),
    ...
    let displacement_pipeline = match create_projection_displacement_pipeline(
        device,
        render_pass,
        pipeline_layout,
        include_bytes!(concat!(
...
unsafe fn create_fullscreen_fragment_pipeline(
    ...
    create_fragment_pipeline_with_vertex(
        device,
        render_pass,
        pipeline_layout,
...
unsafe fn create_projection_displacement_pipeline(
    device: &ash::Device,
    render_pass: vk::RenderPass,
    pipeline_layout: vk::PipelineLayout,
    vertex_spirv: &[u8],
    ...
    if !OPAQUE_PROJECTION_VERTEX_SHADER_COMPILED {
        return Ok(None);
    }
    create_fragment_pipeline_with_vertex(
        device,
...
#[allow(clippy::too_many_arguments)]
unsafe fn create_fragment_pipeline_with_vertex(
    device: &ash::Device,
    render_pass: vk::RenderPass,
    pipeline_layout: vk::PipelineLayout,
    vertex_spirv: &[u8],
    ...
    let vert_module = create_shader_module(device, vertex_spirv, &format!("{label}-vertex"))?;
    let frag_module =
...
unsafe fn push_projection_constants<T>(
    device: &ash::Device,
    command_buffer: vk::CommandBuffer,
    pipeline_layout: vk::PipelineLayout,
    value: &T,
...
mod tests {
    ...
    fn blur_direction_push_steps_match_extent_axes() {
        ...
        assert_eq!(
            PublicGuideBlurDirection::Horizontal.step_and_extent(extent),
            [0.0025, 0.0, 0.0025, 0.005]
        );
        assert_eq!(
            PublicGuideBlurDirection::Vertical.step_and_extent(extent),
            [0.0, 0.005, 0.0025, 0.005]
        );
    }

    #[test]
    fn packed_guide_extent_preserves_the_verified_native_per_eye_resolution() {
        let extent = spatial_public_guide_target_extent();
        assert_eq!(extent.width, 768);
        assert_eq!(extent.height, 384);
        assert_eq!(extent.width / SPATIAL_PUBLIC_PACKED_EYE_COUNT as u32, 384);
        assert_eq!(SPATIAL_PUBLIC_GUIDE_PER_EYE_WIDTH, 384);
        assert_eq!(SPATIAL_PUBLIC_GUIDE_PER_EYE_HEIGHT, 384);
        assert_eq!(mem::size_of::<PublicGuideBlurPush>(), 48);
    }
    ...
    fn packed_projection_target_rects_match_camera_projection_push() {
        let push = crate::camera_hwb_projection_target::camera_hwb_projection_push(1.0);
    ...
    ) -> SpatialPublicDepthViewSnapshot {
        SpatialPublicDepthViewSnapshot {
            fov: SpatialPublicDepthFovSnapshot {
                angle_left: (-horizontal_tangent).atan(),
                angle_right: horizontal_tangent.atan(),
                angle_up: vertical_tangent.atan(),
                angle_down: (-vertical_tangent).atan(),
            },
            pose: SpatialPublicDepthPoseSnapshot::default(),
        }
    }

    fn test_real_depth_binding() -> SpatialPublicDepthBinding {
        let view = test_depth_view(1.0, 1.0);
        SpatialPublicDepthBinding {
            ...
            depth_views: [view; 2],
            render_views: [view; 2],
            depth_view_valid_mask: 0b11,
            render_view_valid_mask: 0b11,
        }
    }

    #[test]
    fn stereo_metadata_affine_is_identity_for_matching_views() {
        let affine = spatial_public_metadata_depth_affine(
            1,
            SpatialPublicDepthLayerPolicy::EyeIndex,
            test_real_depth_binding(),
        );
        assert!(affine.metadata_applied);
        for (actual, expected) in affine
            .row0
            .into_iter()
            .chain(affine.row1)
    ...
    #[test]
    fn stereo_metadata_affine_uses_selected_depth_eye_fov() {
        ...
        assert!(affine.metadata_applied);
        assert!((affine.row0[0] - 0.5).abs() < 0.0001);
        assert!((affine.row1[1] - 0.5).abs() < 0.0001);
        assert!((affine.row0[2] - 0.25).abs() < 0.0001);
        assert!((affine.row1[2] - 0.25).abs() < 0.0001);
    }

    #[test]
    fn manual_depth_alignment_is_centered_and_preserves_metadata_receipt() {
        let affine = compose_spatial_public_manual_depth_alignment(
            SpatialPublicDepthUvAffine {
                row0: [0.5, 0.0, 0.25],
                row1: [0.0, 0.5, 0.25],
                metadata_applied: true,
        ...
        assert!(affine.metadata_applied);
        assert_eq!(affine.row0, [1.0, 0.0, 0.01]);
    ...
    fn packed_projection_scissors_clip_to_native_target_footprint() {
        ...
        let left = target_rect_to_scissor(extent, packed_projection_target_rect(0, 1.0));
        let right = target_rect_to_scissor(extent, packed_projection_target_rect(1, 1.0));
    ...
    fn rect_marker_formats_native_acceptance_tokens() {
        assert_eq!(
            rect_marker(packed_projection_target_rect(0, 1.0)),
            "0.111389;0.218750;0.277778;0.656250"
        );
        assert_eq!(
            rect_marker(packed_projection_target_rect(1, 1.0)),
    ...
    fn guide_pass_schedule_keeps_blur_public() {
        ...
        assert!(public_guide_pass_schedule_marker().contains("public-preblur-horizontal"));
