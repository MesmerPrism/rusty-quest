...
void main() {
  ...
  float driver1_phase = mix(0.08, 0.58, clamp(pc.driver1Value01, 0.0, 1.0));
  float angle = float(index) * 2.39996323 + pc.timeSeconds * (0.22 + driver1_phase);
  float pulse = deformation * sin(pc.timeSeconds * (1.1 + driver1_phase) + float(index) * 0.021);
  float swirl = (0.06 + driver1_phase * 0.16) * sin(pc.timeSeconds * 0.7 + radius * 16.0);
  vec2 position =
  ...
  vec3 color = palette(0.54 + normalized * 0.48 + pc.timeSeconds * (0.014 + driver1_phase * 0.018));
