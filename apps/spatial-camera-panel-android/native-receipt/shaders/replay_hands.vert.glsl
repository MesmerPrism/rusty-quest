...
const float DRIVER0_HIGH_UNIT_DISTANCE_M = 0.004;
const float DRIVER0_LOW_UNIT_DISTANCE_M = 0.0005;
...
vec3 driver_profile_vector(uint anchorIndex, vec3 anchor) {
  ...
  float driver_mix = clamp(pc.dynamics.z, 0.0, 1.0);
  phase += driver_mix * vec3(
...
void apply_driver_profile_dynamics(uint anchorIndex, inout SurfaceParticle particle) {
  particle.driver = driver_profile_vector(anchorIndex, particle.center);
  float highEnergy01 = clamp(pc.profile.y / 0.004, 0.0, 1.0);
  float unitDistance = mix(DRIVER0_LOW_UNIT_DISTANCE_M, DRIVER0_HIGH_UNIT_DISTANCE_M, highEnergy01);
  vec3 driverOffset =
  ...
  particle.center += driverOffset + worldNoise;
}
...
SurfaceParticle live_mesh_particle(uint anchorIndex, vec3 panelForward) {
  ...
  for (uint attempt = 0u; attempt < LIVE_MESH_TRIANGLE_VALIDATION_ATTEMPTS; attempt++) {
    ...
    if (diagnosticMode == 0u) {
      apply_driver_profile_dynamics(anchorIndex, particle);
    }
