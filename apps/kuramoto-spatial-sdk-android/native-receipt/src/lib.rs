...
const NATIVE_MARKER_FILE: &str =
    "/data/data/io.github.mesmerprism.rustyquest.kuramoto_spatial/files/kuramoto_spatial_native_markers.log";
...
pub extern "system" fn Java_io_github_mesmerprism_rustyquest_kuramoto_1spatial_KuramotoSpatialActivity_nativeRecordNoRenderInteropReceipt(
    _env: *mut c_void,
    _thiz: *mut c_void,
...
fn probe_no_present_vulkan_objects(
    ...
    let app_name =
        CString::new("rusty-quest-kuramoto-spatial").expect("static name must not contain NUL");
    let engine_name =
...
fn log_receipt(
    ...
    android_log_info("RQKuramotoSpatialNative", &message);
}

fn log_vulkan_object_probe(openxr_probe: &OpenXrNativeCapabilityProbe) {
    let message = format!(
        "RUSTY_QUEST_KURAMOTO_SPATIAL_NATIVE channel=native-vulkan-object-probe status=observed renderPolicy=no-render vkInstanceCreated={} vkCreateInstanceResult={} vkGraphicsDeviceObtained={} vkGraphicsDeviceResult={} vkGraphicsComputeQueueFound={} vkQueueFamilyIndex={} vkDeviceCreated={} vkCreateDeviceResult={} vkQueueObtained={} vkObjectsDestroyed={} vkDeviceName={} vkDeviceApiVersion={}",
        bool_token(openxr_probe.vulkan_instance_created),
        openxr_probe.vulkan_create_instance_result,
        bool_token(openxr_probe.vulkan_graphics_device_obtained),
        openxr_probe.vulkan_graphics_device_result,
