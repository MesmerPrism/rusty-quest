...
$zoneTests = Read-RequiredText "apps\spatial-camera-panel-android\app\src\test\java\io\github\mesmerprism\rustyquest\spatial_camera_panel\PrivateLayerZoneCompositorTest.kt"
...
Assert-Contains "Profile hotloader" $hotloader "FileObserver("
Assert-Contains "Profile hotloader" $hotloader "sceneTickPolling=false"
...
Assert-Contains "Activity integration" $activity "controlProfileHotloader.poll(force = true)"
Assert-Contains "Activity integration" $activity "CONTROL_PROFILE_HOTLOAD_ENABLED_PROPERTY"
...
Assert-Contains "Zone compositor tests" $zoneTests "nativeAndOrganicStylesPreserveTransparentSpatialVideoUnderlay"
Assert-Contains "Zone compositor tests" $zoneTests "disablingStretchPreservesDynamicBufferAndTransparentSpatialVideoUnderlay"
...
Assert-Contains "Public documentation" $docs "disabled on a normal launch"
Assert-Contains "Public documentation" $docs "file observer"
