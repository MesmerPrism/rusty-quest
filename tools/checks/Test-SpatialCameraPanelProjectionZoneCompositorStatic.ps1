...
Assert-Contains $controls "centerCornerRadiusUv" "Center must expose a normalized corner-radius setting."
...
Assert-Contains $panel '"Corner radius"' "Center geometry must expose the Center-only corner-radius control."
...
Assert-Contains $activity "configuration.centerCornerRadiusUv" "The activity must bridge the live Center corner radius to native code."
...
Assert-Contains $projection "settings.center_corner_radius_uv" "The projection-zone UBO must carry the live Center corner radius."
Assert-Contains $projection "apply_alpha_policy" "The projection-zone frame must apply the effective alpha and unsupported-underlay fallback policy."
...
Assert-Contains $projection "ProjectionZoneCompositeAlpha::Inherit" "The SceneQuadLayer inherited alpha route must retain premultiplied transparent regions."
Assert-Contains $projection "inherited_scene_layer_alpha_keeps_transparent_regions_unsampled" "Inherited Scene-layer transparency needs a native regression test."
...
Assert-Contains $wsi "raw_fallback_markers_bind_record_route_descriptor_and_parity" "Typed Raw fallback evidence needs record-level marker closure fixtures."
Assert-Contains $wsi "effective_zone_alpha_policy.suppresses_same_surface_video(" "The same-surface video draw must follow the alpha-capability-aware policy of the frame actually rendered."
...
Assert-Contains $wsi '"transparent-underlay-fallback-unused"' "A supported direct Spatial underlay must not require a sampled video descriptor."
Assert-Contains $wsi '"retained-video-fallback-unused"' "A hot Video/Transparent change must retain the exact unused video descriptor layout instead of recompiling the compositor pipeline."
Assert-Contains $wsi "retained_unused_descriptor_binding" "The hot no-video compositor route must use the renderer-owned retained descriptor binding."
Assert-Contains $videoProjection "pub(crate) fn retained_unused_descriptor_binding" "The video renderer must expose its retained descriptor only as an unused layout-stability binding."
Assert-Contains $videoProjection "The zone compositor uses this only for shader branches that are proven not to sample video." "The retained descriptor contract must prohibit sampling stale video."
...
Assert-Contains $wsi "vk::CompositeAlphaFlagsKHR::POST_MULTIPLIED" "The Android swapchain must support a straight-alpha fallback without selecting ambiguous inherit first."
Assert-Contains $wsi "projectionZoneSwapchainCompositeAlphaSelected=" "Native evidence must identify only the successfully selected Vulkan swapchain alpha."
Assert-Contains $wsi "projectionZoneSystemLayerBlendObservation=not-observed-by-native-wsi" "Native evidence must not overclaim the Kotlin carrier blend as observed."
Assert-Contains $wsi "projectionZoneShaderAlphaOutputContract=" "Native evidence must identify the configured shader output-alpha convention."
Assert-Contains $wsi "projectionZoneDescriptorRoute=" "Native evidence must identify the selected descriptor decision independently of descriptor availability."
Assert-Contains $wsi "projectionZoneDescriptorAvailable=" "Native evidence must expose descriptor availability before pipeline preparation."
Assert-Contains $wsi "projectionZonePrepareStatus=" "Native evidence must expose the exact pipeline preparation result."
Assert-Contains $wsi "raw_parity_marker_fields" "Raw guide and parity fields must use a compact independent marker."
Assert-Contains $probe "status=projection-zone-raw-parity-effective" "Raw parity evidence must not depend on the longer render-effective marker."
Assert-Contains $projection "transparent_underlay_video_fallback" "Unsupported transparent-underlay alpha must select a readable video fallback."
Assert-Contains $projection "center_transparent_underlay_requested" "Center Transparent must participate in effective underlay policy."
Assert-Contains $projection "middle_transparent_underlay_requested" "Middle transparent reveal must participate in effective underlay policy."
Assert-Contains $projection "outer_transparent_underlay_requested" "Outer Transparent must participate in effective underlay policy."
Assert-Contains $projection "outer_transparent_underlay_supported" "Outer-specific receipt evidence must not be widened to Center or Middle transparency."
Assert-Contains $projection "self.uniform.outer_shape[3] = 0.0" "Legacy v1/v2 unsupported alpha must select effective video through the legacy Outer lane."
Assert-Contains $projection "stretch_flags & !(1 << 13)" "Legacy v2 unsupported alpha must clear the obsolete stretch-extent selection."
Assert-Contains $projection "settings.same_surface_blend_required()" "Same-surface fixed blending must force premultiplied shader output."
...
Assert-Contains $wsi "projectionZoneRenderRoute=" "The renderer must distinguish route ownership from applied zone settings."
Assert-Contains $wsi "projectionZoneAppliedRegionContractVersion=" "The renderer must bind the applied region-contract snapshot."
Assert-Contains $wsi "projectionZoneAppliedCenterContentMode=" "The renderer must bind the applied Center content selection."
Assert-Contains $wsi "projectionZoneAppliedCenterCornerRadiusUv=" "The renderer must bind the applied corner radius."
Assert-Contains $wsi "projectionZoneAppliedBufferFillMode=" "The renderer must bind the applied transition fill."
Assert-Contains $wsi "projectionZoneAppliedOuterContentMode=" "The renderer must bind the applied Outer content selection."
Assert-Contains $wsi "projectionZoneCameraPayloadSource=" "The renderer must bind the camera payload descriptor source."
Assert-Contains $wsi "projectionZoneParityLossReason=" "Raw fallback must expose a typed parity-loss reason."
Assert-Contains $wsi "rawCustomProjectionGuidePassesRecorded=" "Raw evidence must report actual recorded staging passes."
Assert-Contains $wsi "rawCustomProjectionLayerOverride=" "Raw evidence must bind the same per-frame layer snapshot used by guide and projection recording."
Assert-Contains $wsi "rawCustomProjectionDownstreamEffectPassesRecorded=" "Raw evidence must prove downstream guide passes stayed suppressed."
Assert-NotContains $wsi "raw_custom_projection_selected" "Raw camera selection must not regain an independent WSI routing branch."
...
Assert-Contains $runtime "projection_zone_pipeline_binding_compatible" "The video descriptor layout and blend contract must participate in pipeline compatibility."
Assert-Contains $runtime "projection_zone_pipeline_binding_rejects_layout_and_blend_mismatch" "Descriptor layout and blend mismatch need a native damage fixture."
Assert-NotContains $runtime "Raw Projection intentionally skips every guide draw" "Raw camera must not bypass reprojection-aware guide staging."
Assert-Contains $runtime "1 | 6 | 8 => 1" "Raw camera must record exactly guide pass 0 and skip downstream effect passes."
Assert-Contains $runtime "rawCustomProjectionRequestedSource=" "Capability markers must label guide pass 0 as requested rather than effective."
Assert-Contains $runtime "SpatialPublicGuidePassRecord" "Frame recording must return one result that separates requested and actual staging."
Assert-Contains $runtime "frame_layer_override" "Compact projection evidence must consume the immutable layer value returned by frame recording."
Assert-NotContains ([regex]::Match($runtime, '(?s)pub\(crate\) fn compact_projection_evidence_marker_fields\(.*?(?=\r?\n\s*pub\(crate\) fn compact_depth_evidence_marker_fields)').Value) "opaque_projection_layer_override()" "Compact projection evidence must not reread mutable layer state after the WSI frame."
Assert-Contains $runtime "projection_surface_tessellation_allowed(" "Ordinary and shared-zone projection must share the Raw displacement suppression gate."
Assert-Contains $runtime "!camera_content_only && !synthetic_diagnostic" "Raw camera content must force the non-tessellated pipeline."
Assert-Contains $runtime "projection_zone_same_surface_blend_required" "Compositor-owned v4 output must not be blended a second time into its own carrier."
Assert-Contains $runtime "zone_frame.settings.synthetic_diagnostic()," "Synthetic color diagnostics must flow through the shared displacement-suppression gate."
Assert-Contains $wsi "descriptor_routing_and_raw_fallbacks_fail_closed_on_unavailable_inputs" "Unavailable staging and descriptor routes need a native fallback fixture."
