...
Assert-Contains "Immersive coordinator" $coordinator "immersive-video-registration-slot-without-catalog-item"
Assert-Contains "Immersive coordinator" $coordinator "mediaPanelSettings("
Assert-Contains "Immersive coordinator" $coordinator "fun applyRefreshedCatalog("
...
Assert-Contains "Decoder lifecycle nonblocking custom route" $videoRuntime "dispatchDecoderLifecycle"
Assert-Contains "Decoder lifecycle nonblocking custom route" $videoRuntime "uiThreadBlocked=false"
Assert-Contains "Decoder lifecycle nonblocking custom test" $decoderLifecycleTest "regionVisibilityChangeNeverRunsTheBoundedDecoderJoinOnTheCallerThread"
Assert-Contains "Decoder lifecycle generation fence" $decoderLifecycleTest "rapidVideoOffOnSkipsTheStaleStopAndKeepsTheExistingDecoder"
...
Assert-Contains "Immersive validation workflow" $validationWorkflow '"background-video"'
Assert-Contains "Immersive validation workflow" $validationWorkflow 'bindings.setBackgroundMode(SpatialBackgroundMode.Video, source)'
...
Assert-Contains "Immersive UI action tool" $uiActionTool '"background-video"'
