...
Assert-Contains $module 'Guided-breathing bias for next arm'
Assert-Contains $module 'developerGuidanceBias'
Assert-Contains $module 'This is the same 0–100% value shown on the experimenter condition page.'
Assert-Contains $module '(oneReady || twoReady) && !state.hasActiveSession()'
...
Assert-Contains $viewPolicy 'In-app session guard ready'
...
Assert-NotContains $module 'openSelfKioskOverlaySettings'
Assert-Contains $module 'ControlPanelActivity.applyExperimentSessionCommand'
Assert-Contains $module 'polarRuntime.requestStartupPermissions(this)'
Assert-Contains $module 'onRequestPermissionsResult('
Assert-NotContains $module 'maybeRequestStartupKioskPermission()'
Assert-NotContains $module 'viscereality_startup_permissions_v1'
Assert-Contains $module '"session-arm-" + after.generation'
...
Assert-Contains $module 'state.hasActiveSession()'
Assert-Contains $module '!ExperimentSessionPanelViewPolicy.canReturnToImmersive(state)'
...
Assert-Contains $polar 'connectAdmittedCandidate(admittedCandidate, true)'
Assert-Contains $polar 'scheduleAutomaticRecovery'
Assert-Contains $polar 'automaticRecoveryAllowed'
Assert-Contains $polar 'auto-preferred-candidate-admitted'
Assert-Contains $polar 'auto-unique-candidate-admitted'
Assert-Contains $polar 'AUTO_UNIQUE_CANDIDATE_SETTLE_MS'
Assert-Contains $polarRuntime 'requestStartupPermissions(Activity activity)'
Assert-Contains $polarRuntime 'shouldAdmitUniqueCandidateEarly('
Assert-Contains $polarRuntime 'void onHostResume()'
Assert-Contains $module 'PolarSensorRuntime.forApplication(getApplicationContext()).onHostResume()'
