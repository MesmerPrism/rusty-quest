...
$activityMarkers = Read-Required "apps\spatial-camera-panel-android\app\src\main\java\io\github\mesmerprism\rustyquest\spatial_camera_panel\SpatialActivityMarkerRecorder.kt"
$controlProfileHotloader = Read-Required "apps\spatial-camera-panel-android\app\src\main\java\io\github\mesmerprism\rustyquest\spatial_camera_panel\SpatialCameraControlProfileHotloader.kt"
...
Require (-not $activity.Contains('controlProfileHotloader.poll()')) "The XR scene tick must not poll the control-profile filesystem."
Require (-not $activity.Contains('File(filesDir, ACTIVITY_MARKERS_FILE).appendText')) "The XR/activity thread must not append diagnostic marker files."
Require ($activity.Contains('persistToFile = activityMarkerFilePersistenceEnabled()')) "Marker-file persistence must be explicitly enabled."
Require ($activityMarkers.Contains('HandlerThread(WORKER_THREAD_NAME, Process.THREAD_PRIORITY_BACKGROUND)')) "Enabled marker persistence must use a background worker."
Require ($activityMarkers.Contains('existingEvidencePreserved=true') -and $activityMarkers.Contains('MAXIMUM_FILE_BYTES')) "Enabled marker persistence must preserve oversized evidence and enforce a cap."
Require ($controlProfileHotloader.Contains('FileObserver(') -and $controlProfileHotloader.Contains('sceneTickPolling=false')) "Control-profile change detection must be event-driven."
Require ($activity.Contains('CONTROL_PROFILE_HOTLOAD_ENABLED_PROPERTY')) "Control-profile hotload must be explicitly enabled."
