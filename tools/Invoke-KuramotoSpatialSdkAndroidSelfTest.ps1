param(
    [string]$RepoRoot,
    [string]$ApkPath = "target\kuramoto-spatial-sdk-android\rusty-quest-kuramoto-spatial-sdk.apk",
    [string]$OutDir = "",
    [int]$RunSeconds = 19,
    ...
    [string]$Activity = "io.github.mesmerprism.rustyquest.kuramoto_spatial/.KuramotoSpatialActivity",
...
$SelfTestAction = "io.github.mesmerprism.rustyquest.kuramoto_spatial.action.RUN_WORKFLOW_SELF_TEST"
$MarkerPrefix = "RUSTY_QUEST_KURAMOTO_SPATIAL"
...
function Assert-SummaryFlag {
    ...
    if (-not [bool]$Summary[$Name]) {
        throw "Kuramoto Spatial SDK self-test evidence missing required flag: $Name"
    }
}
...
if ([string]::IsNullOrWhiteSpace($OutDir)) {
    ...
    $OutDir = Join-Path $repoRootPath "local-artifacts\kuramoto-spatial-sdk-headset\$stamp-selftest"
...
$summary = [ordered]@{
    '$schema' = "rusty.quest.kuramoto_spatial_selftest_run.v1"
...
try {
    ...
    $tagLines = @($pidLogcat -split "`r?`n" | Where-Object { $_ -match "$MarkerPrefix|RUSTY_QUEST_KURAMOTO_SPATIAL_NATIVE|RQKuramotoSpatial" })
    ...
    $sessionStatePath = Join-Path $appPrivateDir "kuramoto_experiment_session.json"
    $summary.app_private_session_state = Save-AppPrivateFile -RemotePath "files/kuramoto_experiment_session.json" -LocalPath $sessionStatePath
    $sessionId = ""
    ...
    if (-not [string]::IsNullOrWhiteSpace($sessionId)) {
        foreach ($fileName in @(
            ...
            $remoteFile = "files/kuramoto_experiment/$sessionId/$fileName"
    ...
    $summary.condition_handoff_marker = Test-TextContains $pidLogcat "status=experiment-condition-parameter-handoff"
    $summary.self_test_condition_handoff_marker = Test-TextContains $pidLogcat "source=self-test-experiment-block-start"
    $summary.parameter_submit_self_test_marker = (Test-TextContains $pidLogcat "status=parameters-submitted") -and (Test-TextContains $pidLogcat "source=self-test-experiment-block-start")
    ...
    $summary.polar_stream_mirror_registered = Test-TextContains $pidLogcat "streamMirror=kuramoto-experiment-store"
    ...
    $summary.questionnaire_result_row = Test-TextContains $questionnaireText '"schema_id":"rusty.kuramoto.mesh.experiment_questionnaire.v2"'
...
Write-Output "Kuramoto Spatial SDK self-test evidence: $summaryPath"
