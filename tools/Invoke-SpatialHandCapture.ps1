...
switch ($Action) {
    "Prepare" {
        ...
        Set-SpatialProperty "debug.rustyquest.spatial.hand_billboard_flock.enabled" ($(if ($BillboardWireframeEnabled) { "true" } else { "false" }))
        Set-SpatialProperty "debug.rustyquest.spatial.hand_billboard_flock.carrier" "batched-scene-mesh"
        Set-SpatialProperty "debug.rustyquest.spatial.hand_billboard_flock.visual_mode" "wireframe-edges"
