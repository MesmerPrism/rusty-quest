...
$target=Join-Path $repo "target";if([string]::IsNullOrWhiteSpace($OutDir)){$OutDir=Join-Path $target "lslc-005s-rust-float32-two-record-chunk"}
...
$app=Join-Path $repo "apps\lsl-rust-float32-two-record-chunk-android";$classes=Join-Path $fullOut "classes";$dex=Join-Path $fullOut "dex";$native=Join-Path $fullOut "native"
...
@"
[package]
name = "rusty-lsl-float32-two-record-chunk"
version = "0.1.0"
edition = "2024"
[workspace]
[lib]
...
$so=Join-Path $cargoTarget "aarch64-linux-android\release\rusty_lsl_float32_two_record_chunk.dll"
if(-not(Test-Path $so)){$so=Join-Path $cargoTarget "aarch64-linux-android\release\librusty_lsl_float32_two_record_chunk.so"}
...
$unsigned=Join-Path $fullOut "unsigned.apk";$unaligned=Join-Path $fullOut "unaligned.apk";$aligned=Join-Path $fullOut "aligned.apk";$apk=Join-Path $fullOut "rusty-lsl-float32-two-record-chunk.apk"
...
$libDir=Join-Path $fullOut "apk-lib\lib\arm64-v8a";New-Item -ItemType Directory -Force $libDir|Out-Null;Copy-Item $so (Join-Path $libDir "librusty_lsl_float32_two_record_chunk.so")
Checked "native package" $jar @("uf",$unaligned,"-C",(Join-Path $fullOut "apk-lib"),"lib")
Checked "zipalign" $zipalign @("-f","4",$unaligned,$aligned)
if([string]::IsNullOrWhiteSpace($Keystore)){$Keystore=Join-Path $target "lslc-005s-two-record-chunk-debug.keystore"}
if(-not(Test-Path $Keystore)){Checked "keytool" $keytool @("-genkeypair","-keystore",$Keystore,"-storepass","android","-keypass","android","-alias","androiddebugkey","-keyalg","RSA","-keysize","2048","-validity","10000","-dname","CN=Rusty LSL Float32 Two-Record Chunk,O=Rusty Quest,C=US")}
