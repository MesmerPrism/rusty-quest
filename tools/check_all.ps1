...
try {
    ...
    Invoke-Checked "native renderer native passthrough Meta environment depth particles low-capacity profile dry-run" "powershell" @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "tools\Apply-RuntimeProfile.ps1", "-ProfilePath", "fixtures\runtime-profiles\quest-native-renderer-native-passthrough-meta-environment-depth-particles-low-capacity.profile.json", "-DryRun", "-Out", "local-artifacts\native-renderer-native-passthrough-meta-environment-depth-particles-low-capacity-property-write-plan.json")
