...
$PolarLiveAction = "io.github.mesmerprism.rustyquest.kuramoto_spatial.action.RUN_POLAR_LIVE_VALIDATION"
...
function Assert-SummaryFlag {
    ...
    if (-not [bool]$Summary[$Name]) {
        throw "Kuramoto Spatial SDK live Polar evidence missing required flag: $Name"
    }
}
...
if ([string]::IsNullOrWhiteSpace($OutDir)) {
    ...
    $OutDir = Join-Path $repoRootPath "local-artifacts\kuramoto-spatial-sdk-headset\$stamp-polar-live"
...
$summary = [ordered]@{
    '$schema' = "rusty.quest.kuramoto_spatial_polar_live_run.v1"
...
try {
    ...
    $tagLines = @($pidLogcat -split "`r?`n" | Where-Object { $_ -match "$MarkerPrefix|RQKuramotoSpatial|BluetoothGatt" })
    ...
    $activityMarkersPath = Join-Path $appPrivateDir "kuramoto_spatial_activity_markers.log"
    $summary.app_private_activity_markers = Save-AppPrivateFile -RemotePath "files/kuramoto_spatial_activity_markers.log" -LocalPath $activityMarkersPath
...
Write-Output "Kuramoto Spatial SDK live Polar evidence: $summaryPath"
