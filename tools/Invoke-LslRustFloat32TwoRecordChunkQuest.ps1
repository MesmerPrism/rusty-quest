...
$ErrorActionPreference="Stop";$package="io.github.mesmerprism.rustyquest.lslrustfloat32tworecordchunk";$component="$package/.Float32TwoRecordChunkActivity"
$manifest=Get-Content -Raw $BuildManifest|ConvertFrom-Json;$apk=Join-Path(Split-Path $BuildManifest)"rusty-lsl-float32-two-record-chunk.apk"
...
try{
    ...
    if($result.result-ne"pass"-or$result.native_result-ne1){throw "Native runtime result failed"};if(-not$logs.Contains("RLSL005S_RUST")-or-not$logs.Contains('rusty.lsl.rust_on_quest_float32_two_record_chunk.v1')-or-not$logs.Contains('"record_count":2')-or-not$logs.Contains('"timestamp_bits":["0x8000000000000000","0x40a255c000000000"]')-or-not$logs.Contains('"value_bits":["0x7fc01234","0xc0200000"]')-or-not$logs.Contains('"port_reuse":true')){throw "Rust effective marker missing"}
    ...
    [ordered]@{schema="rusty.quest.lsl_rust_float32_two_record_chunk_device.v1";result="pass";serial=$Serial;build_manifest_sha256=(Get-FileHash -Algorithm SHA256 $BuildManifest).Hash.ToLowerInvariant();rust_effective_marker=$true;java_lifecycle_marker=$true;record_count=2;ordered_exact_bits=$true;port_reuse=$true;bounded_fatal_count=0;prior_package_present=$false;cleanup="pending"}|ConvertTo-Json|Set-Content -Encoding UTF8(Join-Path $OutDir "private-device-receipt.json")
}finally{
