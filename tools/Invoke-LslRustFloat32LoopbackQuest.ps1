...
$ErrorActionPreference="Stop";$package="io.github.mesmerprism.rustyquest.lslrustfloat32loopback";$component="$package/.Float32LoopbackActivity"
$manifest=Get-Content -Raw $BuildManifest|ConvertFrom-Json;$apk=Join-Path(Split-Path $BuildManifest)"rusty-lsl-float32-loopback.apk"
...
try{
    ...
    if($result.result-ne"pass"-or$result.native_result-ne1){throw "Native runtime result failed"};if(-not$logs.Contains("RLSL005L_RUST")-or-not$logs.Contains('rusty.lsl.rust_on_quest_float32_loopback.v1')-or-not$logs.Contains('"port_reuse":true')){throw "Rust effective marker missing"}
    ...
    [ordered]@{schema="rusty.quest.lsl_rust_float32_loopback_device.v1";result="pass";serial=$Serial;build_manifest_sha256=(Get-FileHash -Algorithm SHA256 $BuildManifest).Hash.ToLowerInvariant();rust_effective_marker=$true;java_lifecycle_marker=$true;exact_bits=$true;port_reuse=$true;bounded_fatal_count=0;prior_package_present=$false;cleanup="pending"}|ConvertTo-Json|Set-Content -Encoding UTF8(Join-Path $OutDir "private-device-receipt.json")
}finally{
