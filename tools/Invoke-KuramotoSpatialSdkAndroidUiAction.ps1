...
$UiCommandAction = "io.github.mesmerprism.rustyquest.kuramoto_spatial.action.RUN_UI_COMMAND"
...
$launch = Invoke-AdbCommand -Name "run Kuramoto Spatial UI action $Action" -Arguments $intentArguments
...
if ($ReadMarkers) {
    $markerResult = Invoke-AdbCommand `
        ...
        -Arguments @("exec-out", "run-as", $PackageName, "tail", "-n", "40", "files/kuramoto_spatial_activity_markers.log") `
        -AllowFailure
    $markerTail = $markerResult.output
}

[pscustomobject]@{
    schema = "rusty.quest.kuramoto_spatial_ui_action_invoked.v1"
    serial = $Serial
