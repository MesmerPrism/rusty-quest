...
$target=Join-Path $repo "target";if([string]::IsNullOrWhiteSpace($OutDir)){$OutDir=Join-Path $target "lslc-005h-rust-on-quest"}
...
$app=Join-Path $repo "apps\lsl-rust-conformance-android";$classes=Join-Path $fullOut "classes";$dex=Join-Path $fullOut "dex";$native=Join-Path $fullOut "native"
...
$unsigned=Join-Path $fullOut "unsigned.apk";$unaligned=Join-Path $fullOut "unaligned.apk";$aligned=Join-Path $fullOut "aligned.apk";$apk=Join-Path $fullOut "rusty-lsl-rust-conformance.apk"
...
$libDir=Join-Path $fullOut "apk-lib\lib\arm64-v8a";New-Item -ItemType Directory -Force $libDir|Out-Null;Copy-Item $so (Join-Path $libDir "librusty_lsl_quest_conformance.so")
Checked "native package" $jar @("uf",$unaligned,"-C",(Join-Path $fullOut "apk-lib"),"lib")
Checked "zipalign" $zipalign @("-f","4",$unaligned,$aligned)
if([string]::IsNullOrWhiteSpace($Keystore)){$Keystore=Join-Path $target "lslc-005h-debug.keystore"}
if(-not(Test-Path $Keystore)){Checked "keytool" $keytool @("-genkeypair","-keystore",$Keystore,"-storepass","android","-keypass","android","-alias","androiddebugkey","-keyalg","RSA","-keysize","2048","-validity","10000","-dname","CN=Rusty LSL Rust Quest Conformance,O=Rusty Quest,C=US")}
...
$manifest=[ordered]@{schema="rusty.quest.lsl_rust_conformance_build.v1";quest_source_head=$questHead;rusty_lsl_source_head=$lslHead;quest_source_clean=$true;rusty_lsl_source_clean=$true;rust_target="aarch64-linux-android";package="io.github.mesmerprism.rustyquest.lslrustconformance";activity=".RustConformanceActivity";apk_sha256=(Get-FileHash -Algorithm SHA256 $apk).Hash.ToLowerInvariant();native_sha256=(Get-FileHash -Algorithm SHA256 $so).Hash.ToLowerInvariant()}
