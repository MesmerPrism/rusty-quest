...
param(
    ...
    [Parameter(Mandatory = $true)]
    [ValidateSet(
        "spatial-sdk-broker-retired-fence-complete",
        "vulkan-wsi-queue-present-returned")]
...
function ConvertFrom-FreshnessMarker {
    ...
    $expectedSessionAuthority = if ($authority -ceq "spatial-sdk-broker-retired-fence-complete") {
        "spatial-sdk-device-binding"
    } elseif ($authority -ceq "vulkan-wsi-queue-present-returned") {
        "app-vulkan-wsi-run"
    } else {
        throw "Freshness receipt cadence authority is unsupported."
    }
    $expectedPresentAuthority = if ($authority -ceq "spatial-sdk-broker-retired-fence-complete") {
        "app-submit-ordinal-retired-by-sdk-broker"
    } else {
        "vulkan-wsi-queue-present-returned"
    }
