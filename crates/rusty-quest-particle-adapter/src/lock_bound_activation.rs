//! Particle-adapter policy facade over the application-neutral lock engine.
...
pub use rusty_quest_feature_activation::{
    LockBoundActivationRejection as ParticleAdapterLockRejection,
    LockBoundActivationRuntimeInput as ParticleAdapterRuntimeActivationInput,
    LockBoundActivationState as ParticleAdapterLockActivationState,
    LOCK_BOUND_ACTIVATION_SCHEMA_ID,
};

use crate::PARTICLE_ADAPTER_RECEIPT_SCHEMA_ID;

const PARTICLE_ADAPTER_EFFECTIVE_MARKER_ID: &str = "rusty.quest.particle_adapter.effective";
const PARTICLE_ADAPTER_REQUESTED_BY: &str = "conformance-profile:particle-adapter";

/// Nominal particle-only activation authority minted by this facade.
///
/// The inner generic decision is private so a hand decision or a manually
/// assembled applied state cannot cross the particle effect gate.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ParticleAdapterLockActivationDecision {
    inner: LockBoundActivationDecision,
}

impl ParticleAdapterLockActivationDecision {
    /// Whether particle marker and data effects may proceed.
    #[must_use]
    ...
    #[must_use]
    pub fn rejection(&self) -> Option<ParticleAdapterLockRejection> {
...
///
/// This facade deliberately owns only particle-specific selection policy. JSON
...
#[must_use]
pub fn resolve_particle_adapter_activation(
    lock_json: &str,
    ...
    runtime_input: &ParticleAdapterRuntimeActivationInput,
    ...
    ParticleAdapterLockActivationDecision {
        inner: resolve_lock_bound_activation(
            ...
            LockBoundActivationPolicy {
                requested_by: PARTICLE_ADAPTER_REQUESTED_BY,
                receipt_schema: PARTICLE_ADAPTER_RECEIPT_SCHEMA_ID,
                effective_marker: PARTICLE_ADAPTER_EFFECTIVE_MARKER_ID,
            },
            runtime_input,
        ),
    }
...
mod tests {
    ...
    const FEATURE: &str = "particle-adapter-consumer";
    const PROFILE: &str = "profile.test.particle-adapter";

    fn lock(enabled: bool) -> String {
        format!(
            r#"{{"schema":"rusty.morphospace.workflow.feature_lock.v1","project_id":"{PROJECT}","revision":7,"default_activation":"disabled","features":[{{"feature_id":"{FEATURE}","module_id":"{FEATURE}","enabled":{enabled},"requested_by":"conformance-profile:particle-adapter","descriptor":"morphospace/project.spec.json#{FEATURE}","dependencies":[],"conflicts":[],"permissions":[],"routes":[],"assets":[],"parameter_authorities":[],"activation_receipt":{{"required":true,"schema":"rusty.quest.particle_adapter.receipt.v1","effective_marker":"rusty.quest.particle_adapter.effective"}}}}]}}"#
        )
    }

    fn input(lock: &str) -> ParticleAdapterRuntimeActivationInput {
    ...
    fn default_disabled_lock_rejects_before_runtime_input() {
        ...
        assert_eq!(
            decision.rejection(),
            Some(ParticleAdapterLockRejection::FeatureNotSelected)
        );
        assert!(!decision.is_applied());
    }
    ...
    fn stale_wrong_and_unaccepted_runtime_inputs_reject() {
        ...
        cases.push((disabled, ParticleAdapterLockRejection::RuntimeInputDisabled));
        let mut profile = baseline.clone();
        profile.profile_id = "profile.unaccepted".to_owned();
        cases.push((
            profile,
            ParticleAdapterLockRejection::RuntimeProfileMismatch,
        ));
        let mut project = baseline.clone();
        project.project_id = "other-project".to_owned();
        cases.push((
            project,
            ParticleAdapterLockRejection::RuntimeProjectMismatch,
        ));
        let mut feature = baseline.clone();
        feature.feature_id = "other-feature".to_owned();
        cases.push((
            feature,
            ParticleAdapterLockRejection::RuntimeFeatureMismatch,
        ));
        let mut revision = baseline.clone();
        revision.lock_revision = 6;
        cases.push((
            revision,
            ParticleAdapterLockRejection::RuntimeRevisionMismatch,
        ));
        let mut digest = baseline;
        digest.lock_sha256 = "0".repeat(64);
        cases.push((digest, ParticleAdapterLockRejection::RuntimeDigestMismatch));
