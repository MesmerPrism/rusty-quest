...
The only admitted cadence tuple is the app-owned Vulkan WSI run with successful
`queue_present` return authority; retired Spatial SDK broker cadence is not a
compatible or fallback receipt source.
